...
bool DataStore::loadSensorIdentity(mesh::LocalIdentity &identity) {
  return identity_store.load("_sensor", identity);
}

bool DataStore::saveSensorIdentity(const mesh::LocalIdentity &identity) {
  return identity_store.save("_sensor", identity);
}

void DataStore::loadPrefs(ClientNodePrefs &prefs, CommonCLI &cli, double node_lat, double node_lon) {
  if (_fs->exists("/new_prefs")) {
    loadPrefsInt("/new_prefs", prefs, node_lat, node_lon); // new filename
  } else if (_fs->exists("/node_prefs")) {
    loadPrefsInt("/node_prefs", prefs, node_lat, node_lon);
    savePrefs(prefs, cli, node_lat, node_lon); // save to new filename
    _fs->remove("/node_prefs");                // remove old
  }
  cli.loadPrefs(_fs);
}

void DataStore::loadPrefsInt(const char *filename, ClientNodePrefs &_prefs, double &node_lat,
                             double &node_lon) {
...
void DataStore::savePrefs(const ClientNodePrefs &_prefs, CommonCLI &cli, double node_lat, double node_lon) {
